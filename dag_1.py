...
# schedule_interval = é o periodo de execução
# catchup = é se ele vai executar as tarefas que deram certo mesmo depois de uma ter falhado.
# o get_max_primary_key = é a captação da maior chave primaria contida na tabela
# as conexões dos providers pode ser feita no snowflake e trazida para dentro do código
...
def postgres_to_snowflake_etl():
    """This DAG loads data incrementally from Postgres to Snowflake."""
    table_names = [
        "veiculos",
        "estados",
        "cidades",
        ...
        def load_incremental_data(table_name: str, max_id: int):
            with PostgresHook(postgres_conn_id="postgres").get_conn() as pg_conn:
                with pg_conn.cursor() as pg_cursor:
                    ...
                    # aqui estamos utilizando a tabela criada pelo postgres para armazenar o schema do banco de dados.
                    pg_cursor.execute(
                    ...
                    pg_cursor.execute(
                        f"SELECT {columns_list_str} FROM {table_name} WHERE {primary_key} > {max_id}"
                    )
